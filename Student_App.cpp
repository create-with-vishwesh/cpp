...
class Student
{
    ...
    public:
        ...
        Student(int id,int m1,int m2,int m3,int m4,int m5,int m6,int m7,const char Name[],char grade){
            studentid=id;
            phy=m1;
            chem=m2;
            maths=m3;
...
{
    cout<<"\nID\tName\tPhy\tChem\tMaths\tEng\tGuj\tHindi\tComp\tTotal\tPer\tGrade\n";
...
int main()    
{
    ...
    s.scandetails();
    s.displaydetails();
    Student s1(1,90,90,90,90,90,90,90,"Scholar",'A');
    s1.displaydetails();
    return 0;
}
